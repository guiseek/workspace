...
import { EventWithTarget } from '../paper-forms.types';

/**
 * Keyboard events
 */
const isArrowUp = (kb: number) => kb === 38;
const isArrowDown = (kb: number) => kb === 40;
const isArrowUpDown = (kb: number) => isArrowUp(kb) || isArrowDown(kb);
const isEnter = (kb: number) => kb === 13;
const isBackspace = (kb: number) => kb === 8;
const isDelete = (kb: number) => kb === 46;
const isESC = (kb: number) => kb === 27;
const isTab = (kb: number) => kb === 9;

@Component({
  selector: 'ux-autocomplete',
...
export class AutocompleteComponent
  ...
  @HostBinding('class.ux-autocomplete')
  public uxAutocompleteStyle = true;
  ...
  @Input() public notFoundText = ': /'; // set custom text when filter returns empty result
   ...
   */
  onChange(event: EventWithTarget<HTMLInputElement>) {
